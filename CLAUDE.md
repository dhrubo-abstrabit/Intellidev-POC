...
- `.env.local`'s `NANGO_SERVER_URL`/`NANGO_SECRET_KEY`/`NEXT_PUBLIC_NANGO_HOST`/`NEXT_PUBLIC_NANGO_CONNECT_URL` must match whatever Vercel's env has — this project currently points local dev and production at the *same* self-hosted Nango environment (see NANGO_MIGRATION_LOG.md), not separate per-environment secrets. A mismatched `NANGO_SECRET_KEY` fails as a 401 from Nango's own proxy, not a decrypt error — the old hand-rolled AES-256-GCM token layer (and its `TOKEN_ENC_KEYS` matching requirement) is gone; Nango owns token custody entirely now.
...
- Self-hosted Nango owns the OAuth handshake and token custody for every connector below — there is no `/api/oauth/[provider]/callback` route, no per-provider authorize-URL builder, and no local token encryption anymore (all deleted; see NANGO_MIGRATION_LOG.md for the full migration rationale, D-001 onward). Connecting a provider opens Nango's own Connect UI as a popup (`src/components/dashboard/connect-provider-button.tsx`), not a redirect to the provider.

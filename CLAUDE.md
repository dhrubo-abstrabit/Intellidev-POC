...
- `include_granted_scopes` is intentionally never set. Turning it on would carry every previously-granted scope forward onto a new token — e.g. connecting Gmail after Drive would hand the Gmail credential Drive access too. Three connectors, three separate least-privilege consent screens.

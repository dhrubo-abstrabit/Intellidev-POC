...
- `.env.local`'s `NANGO_SERVER_URL`/`NANGO_SECRET_KEY`/`NEXT_PUBLIC_NANGO_HOST`/`NEXT_PUBLIC_NANGO_CONNECT_URL` must match whatever Vercel's env has — this project currently points local dev and production at the *same* self-hosted Nango environment (see docs/frontend/NANGO_MIGRATION_LOG.md), not separate per-environment secrets. A mismatched `NANGO_SECRET_KEY` fails as a 401 from Nango's own proxy, not a decrypt error — the old hand-rolled AES-256-GCM token layer (and its `TOKEN_ENC_KEYS` matching requirement) is gone; Nango owns token custody entirely now.
...
## Connector OAuth is Nango's job now, not this app's

- Self-hosted Nango owns the OAuth handshake and token custody for every connector below — there is no `/api/oauth/[provider]/callback` route, no per-provider authorize-URL builder, and no local token encryption anymore (all deleted; see docs/frontend/NANGO_MIGRATION_LOG.md for the full migration rationale, D-001 onward). Connecting a provider opens Nango's own Connect UI as a popup (`src/components/dashboard/connect-provider-button.tsx`), not a redirect to the provider.
- Free self-hosted Nango has **no webhooks** — the only signal that a connection succeeded is the Connect UI's client-side `connect` event. `reconcileConnections` (`integrations/actions.ts`) is the fallback for when that event never arrives (tab closed mid-flow): it's fired from the Connect UI's `close`-without-`connect` event and from a manual "Check for connections" button, not on every Integrations page load — that used to put an uncached Nango HTTP call on the page's critical path for a check that almost always finds nothing.
- A scope added to a connector's scope list (`src/connectors/slack/index.ts`'s `BOT_SCOPES`, `src/connectors/google/scopes.ts`) does **not** retroactively upgrade an already-connected grant, same as the old hand-rolled OAuth had — an already-connected account needs an explicit disconnect + reconnect. The difference is *where* the scope actually has to be added: now it's the provider's own OAuth app dashboard (Slack: OAuth & Permissions → Bot Token Scopes; Google: the consent screen's scope list) **and** the corresponding integration's scope config inside Nango's dashboard — editing this app's scope-list constants alone is documentation, not enforcement, since Nango builds the authorize URL now, not this app.
- `finalizeConnectionCore`'s `missingScopes()` check (`integrations/actions.ts`) verifies a connection actually got everything it asked for by parsing Nango's returned `raw.scope` string — Nango's connection-read API has no dedicated `granted_scopes` field. It throws inline as a normal error surfaced on the Integrations page, not a dedicated error code.
...
- One Nango integration named `google` backs the connector — its OAuth client ID/secret and scope list are configured entirely inside Nango's dashboard now, not as env vars this app reads (the old `GOOGLE_CONNECTOR_CLIENT_ID/SECRET`, distinct from `GOOGLE_OAUTH_CLIENT_ID/SECRET` which is Supabase Auth *sign-in* and unaffected by any of this, are gone). `getConnector("google").nangoProviderConfigKey` is what ties this app's connector id to that Nango integration id. Console setup: enable the Gmail/Drive/Chat **and People** APIs (People is easy to miss — it's not one of the three "connector" APIs, but Chat's sender-name resolution needs it; see below), create a **Web application** OAuth client, publish the consent screen **Internal** to the Workspace org, and register `{NANGO_SERVER_URL}/oauth/callback` as the redirect URI — Nango owns exactly one callback, on its own server, not one per app origin. The old per-service redirect URIs (`.../api/oauth/{gmail,google_drive,google_chat}/callback`) and the later merged-connector one (`.../api/oauth/google/callback`) are both gone along with the rest of the hand-rolled OAuth layer.
- `access_type=offline`/`prompt=consent`-equivalent behavior (getting a `refresh_token` on every grant, including reconnects) is Nango's concern now, not this app's — there's no authorize-URL code left here to set them on. If a Google reconnect ever stops refreshing correctly, look at the integration's config in Nango's dashboard, not this app's connector code.
- The scope-changes-need-reconnect lesson still applies to Google specifically via `finalizeConnectionCore`'s `missingScopes()` check (see "Connector OAuth is Nango's job now" above) — it throws as a normal Integrations-page error, not the old dedicated `MISSING_SCOPES` code `exchangeGoogleCode` used to throw (that function is deleted).
- `include_granted_scopes` is a setting on Nango's `google` integration now (see `connectors/google/scopes.ts`'s doc comment) — still intentionally off, for the same reason as before: the merged connector's requested list should stay the *whole* truth about what a token can do, not silently accrete whatever else that Google account happened to grant this client earlier.

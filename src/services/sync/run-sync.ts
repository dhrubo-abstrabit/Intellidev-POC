...
import { openTokens, fromBytea, toBytea } from "@/lib/crypto/tokens";
...
type IntegrationRow = Pick<
  ...
  "id" | "workspace_id" | "project_id" | "provider" | "credential_id" | "sync_interval_seconds" | "consecutive_failures"
>;
...
async function loadCredentials(service: ServiceClient, integration: IntegrationRow): Promise<ConnectorCredentials> {
  ...
  const { data: credentialRow, error } = await service
    ...
    .select("id, secret_ciphertext, secret_iv, secret_key_version, secret_alg, external_account_id, external_account_label")
...
export async function runSync(
  ...
  const { data: integration } = await service
    .from("integrations")
    .select("id, workspace_id, project_id, provider, credential_id, sync_interval_seconds, consecutive_failures")
    .eq("id", integrationId)
    .maybeSingle();
  if (!integration) {
    return { status: "failed", eventsFetched: 0, eventsWritten: 0, error: "Integration not found" };
  }
  ...
  if (jobError || !job) {
    ...
    return { status: "skipped", eventsFetched: 0, eventsWritten: 0, error: jobError?.message };
  }
  ...
  try {
    ...
    if (rawRows.length > 0) {
      ...
      const { data: existingRaw } = candidateProviderEventIds.length
        ? await service
            .from("raw_events")
            .select("provider_event_id")
            .eq("integration_id", integration.id)
            .in("provider_event_id", candidateProviderEventIds)
        : { data: [] as { provider_event_id: string | null }[] };
      const alreadySeen = new Set((existingRaw ?? []).map((r) => r.provider_event_id));
      const newRawRows = rawRows.filter((r) => r.provider_event_id === null || !alreadySeen.has(r.provider_event_id));

      const { data: insertedRaw, error: rawError } =
        newRawRows.length > 0
          ? await service.from("raw_events").insert(newRawRows).select("id, provider_event_id")
          : { data: [], error: null };
    ...
    return { status: "succeeded", eventsFetched: fetchResult.rawPayloads.length, eventsWritten };
  } catch (err) {
    ...
    return { status: "failed", eventsFetched: 0, eventsWritten: 0, error: message };
  }
}

...
import { MembersView, type MemberRow, type AssignableRole } from "./members-view";

/**
 * Workspace membership management.
 *
 * Note there is no role check anywhere in this file. Whether the viewer may
 * manage membership is answered entirely by `assignable_roles` coming back
 * non-empty — that RPC applies the member.manage check, the `assignable`
 * flag and the rank ceiling in one place, server-side. A hard-coded set like
...
export default async function MembersPage({ params }: { params: Promise<{ workspaceId: string }> }) {
  ...
  const [{ data: memberRows }, { data: assignable }, { data: roleCatalog }] = await Promise.all([
    supabase

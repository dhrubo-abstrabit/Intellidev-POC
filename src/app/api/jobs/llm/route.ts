...
interface LlmJobPayload {
  ...
  /** Client-space-local calendar day ("2026-08-10") to extract action items
   * for. Computed by the caller (run-sync.ts / batch.ts) at enqueue time,
   * not defaulted here — delivery isn't instant, so a job enqueued right
...
async function handler(request: NextRequest) {
  ...
  const result = await generateActionItems(body.clientSpaceId, body.date);

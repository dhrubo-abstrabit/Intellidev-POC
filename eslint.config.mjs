...
const eslintConfig = defineConfig([
  ...
  globalIgnores([
    ...
    "supabase/.temp/**",
    "supabase/.branches/**",
    // The control plane is its own monorepo, vendored as a subtree. It has no ESLint of
    // its own — it is held by prettier, tsc and its test suite — and flat config lints the
    // whole directory by default, so without this line `npm run lint` fails on 2 errors and
    // 18 warnings from code this config was never written for. Same reason `control-plane`
    // is excluded in tsconfig.json.
    "control-plane/**",
  ]),
]);
